        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="flex items-center space-x-2 bg-brand-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-brand-700 transition-all shadow-lg shadow-brand-500/20 text-sm"
                    >
                        <Plus size={18} />
            ...
            {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {[1, 2, 3, 4, 5, 6].map(i => (
                        <div key={i} className="card-saas h-96 animate-pulse bg-slate-50 border border-slate-100 rounded-2xl" />
                    ))}
                </div>
                                ...
                                <div className="bg-slate-50/50 rounded-xl p-4 border border-slate-100">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-baseline space-x-1">
                                            <span className="text-xl font-bold text-slate-900">₹{product.prices?.['500g'] || '0'}</span>
                                            <span className="text-[10px] text-slate-500 font-medium">/ 500g</span>
                                        </div>
                                        <div className="flex items-center text-brand-600 font-bold text-xs group/link cursor-pointer">
                ...
                {showModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm">
                        ...
                        >
                            <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                                <div>
                                    <h2 className="text-xl font-bold text-slate-900">
                                        {editingProduct ? 'Update Product Details' : 'Configure New Product'}
                                    </h2>
                                    <p className="text-xs text-slate-500 mt-0.5">Define SKU and catalog positioning attributes.</p>
                                </div>
                                <button
                            ...
                            <form id="product-form" onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-8 custom-scrollbar">
                                    ...
                                    <div className="space-y-6">
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                ...
                                                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500/10 focus:border-brand-500 outline-none transition-all"
                                                value={formData.name}
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <select
                                                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500/10 focus:border-brand-500 outline-none transition-all appearance-none cursor-pointer"
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <textarea
                                                rows={5}
                                                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-brand-500/10 focus:border-brand-500 outline-none transition-all resize-none"
                                                value={formData.description}
                                                onChange={e => setFormData({ ...formData, description: e.target.value })}
                                                placeholder="Elaborate on ingredients, flavor profile, and unique selling points..."
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-8">
                                        <div className="space-y-4">
                                            ...
                                            <div className="flex items-center gap-6">
                                                <div className="relative group w-40 h-40 rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 overflow-hidden flex flex-col items-center justify-center transition-all hover:bg-slate-100 hover:border-slate-300">
                                                    {formData.image_url ? (
                                                    ...
                                                    {formData.image_url && (
                                                        <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
                                                            <p className="text-[10px] font-bold text-white uppercase tracking-widest">{uploading ? 'Processing...' : 'Change Asset'}</p>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="flex-grow space-y-3">
                                                    <div className="relative">
                                                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 block">Custom Image Identifier / URL</label>
                                                        <input
                                                            className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono focus:ring-2 focus:ring-brand-500/10 outline-none transition-all"
                                                            value={formData.image_url}
                                                            onChange={e => setFormData({ ...formData, image_url: e.target.value })}
                                                            placeholder="https://cloud.storage/asset.png"
                                                        />
                                                    </div>
                                                    <p className="text-[9px] text-slate-400 leading-relaxed font-medium">Recommended: High resolution 1:1 aspect ratio. Maximum file size: 5MB.</p>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="p-6 bg-slate-50 rounded-2xl border border-slate-200">
                                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-4 text-center">Unit-Based Pricing Structure (INR)</label>
                                            <div className="grid grid-cols-5 gap-3">
                                                {weights.map(size => (
                                                    <div key={size} className="space-y-2">
                                                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-center">{size}</p>
                                                        <input
                                                            type="number"
                                                            required
                                                            className="w-full p-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-500/20 outline-none font-bold text-center text-slate-700 text-xs shadow-sm"
